...
namespace SplineNaK
{
	...
	public:
		...
		double deriv(double x_s) const
		{
		    auto it = std::lower_bound(xp->begin(), xp->end(), x_s);
		    int idx = (std::max)(int(it - xp->begin()) - 1, 0);
		    idx = (std::min)(idx, int(xp->size() - 2));
		
		    const double x0 = (*xp)[idx];
		    const double x1 = (*xp)[idx + 1];
		    const double h  = x1 - x0;
		    const double t  = x_s - x0;
		
		    // Your evaluation corresponds to standard cubic:
		    // y = a + b*t + (c - d*h)*t^2 + d*t^3
		    const double cstd = c[idx] - d[idx] * h;
		
		    return b[idx] + 2.0 * cstd * t + 3.0 * d[idx] * t * t;
		}
	
		double deriv2(double x_s) const
		{
		    auto it = std::lower_bound(xp->begin(), xp->end(), x_s);
		    int idx = (std::max)(int(it - xp->begin()) - 1, 0);
		    idx = (std::min)(idx, int(xp->size() - 2));
